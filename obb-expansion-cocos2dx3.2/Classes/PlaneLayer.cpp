...
bool PlaneLayer::onTouchBegan(Touch* touch, Event* pEvent) {
    ...
    bool f = target->getBoundingBox().containsPoint(touch->getLocation());
    
    log("f=%s", f ? "true" : "false");
#ifdef USE_SCREEN_LOG
    g_screenLog->log( LL_INFO, "●●● f=%s ●●●", f ? "true" : "false" );
#endif
    ...
    Vec2 mTargetPoint = target->getPosition();
    Vec2 mTouchPoint = touch->getLocation();
    
    log("mTouchPoint.x=%f,mTouchPoint.y=%f",mTouchPoint.x,mTouchPoint.y);
    log("mTargetPoint.x=%f,mTargetPoint.y=%f",mTargetPoint.x,mTargetPoint.y);
    log("locationInNode.x=%f,locationInNode.y=%f",locationInNode.x,locationInNode.y);
